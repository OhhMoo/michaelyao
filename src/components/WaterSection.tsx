  ...
  return (
    <section className="npage npage--water" id="water">
      ...
      {/* Everything below shares one `.essay` ancestor so the sidenote counter
          numbers continuously across all three prose sections. */}
      <div className="essay">
        ...
        <FadeIn className="np-col">
          ...
          <p>
            ...
            <Sidenote>
              TIP4P/2005 and TIP5P are rigid, point-charge parameterizations of the water molecule
              built for molecular-dynamics simulation — along with SWM4-NDP, the third model used in
              the generality check below, these are three independently developed descriptions of
              the same substance.
            </Sidenote>{" "}
            ...
            <Sidenote>
              DBSCAN is a density-based clustering step that strips out sparse, ambiguous points
              without requiring a fixed cluster count in advance; the GMM (Gaussian mixture model)
              that follows then fits soft, probabilistic memberships to whatever populations DBSCAN
              leaves behind.
            </Sidenote>{" "}
        ...
        <FadeIn className="np-col">
          ...
          <p>
            ...
            <Sidenote>
              &ldquo;Model-independent&rdquo; here means the same two-population split shows up
              whether you look at the four order parameters used to build the clusters or at S(k)
              computed straight from atomic coordinates — since the clustering step and the
              validation step never share a descriptor, one can&apos;t have quietly leaked into the
              other.
            </Sidenote>
          </p>
        </FadeIn>
